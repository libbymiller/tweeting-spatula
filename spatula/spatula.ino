//mostly copied from the amazing https://github.com/soramimi/ESP8266Tweet
...
void loop() {
  ...
  if(event.gyro.x > threshold && event.gyro.y > threshold && event.gyro.z > threshold){
    ...
    std::string text ="Jazz spatula is cookin' (x:";
    text += itoa((event.gyro.x * 100),x,10);
    text += " y:";
    text += itoa((event.gyro.y * 100),y,10);
    text += " z:";
    text += itoa((event.gyro.z * 100),z,10);
    text += ")";
    tc.tweet(text);
    digitalWrite(LED, 0);
    delay(500);
